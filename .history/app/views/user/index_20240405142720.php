            ...
            <div class="row">
               <div class="col-3">
                  <select name="status" id="" class="form-control">
                     ...
                     <option value="active" {{$request->getFields()['status'] == 'active' ? 'checked' : false}}>Kích hoạt</option>
                     <option value="inactive" {{$request->getFields()['status'] == 'inactive' ? 'checked' : false}}>Chưa kích hoạt</option>
                  </select>
               </div>
               <div class="col-3">
                  <select name="group_id" id="" class="form-control">
                     ...
                     @if(!empty($listGroups))
                        @foreach ($listGroups as $group)
                           <option value="{{$group['id']}}" {{$request->getFields()['group_id'] == $group['id'] ? 'checked' : false}}>{{$group['name']}}</option>
                        @endforeach
                     @endif
                  </select>
               </div>
