...
<div class="col-9">
      ...
      <tbody>
         @if(!empty($listUsers))
            @foreach($listUsers as $user)
               <tr>
                  <td><input type="checkbox" name="" class="check-item-user"></td>
                  <td>{{$user["fullname"]}}</td>
                  <td>{{$user["email"]}}</td>
   ...
   <div class="row">
      <div class="col-6">
         <button type="button" class="btn btn-danger btn-delete-multi-user disabled">Xóa đã chọn (<span>0</span><i></i>)</button>
      </div>
      <div class="col-6">
         {!$links!}
      </div>
