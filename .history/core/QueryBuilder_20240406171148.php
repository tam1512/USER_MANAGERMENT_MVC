...
trait QueryBuilder {
   ...
   private function getPaginateView($limit, $page) {
      ...
      $html = '<nav class="d-flex justify-content-end"><ul class="pagination"><li class="page-item"><a class="page-link '.($isDisablePrev ? 'disabled' : false).'" href="'._WEB_HOST_ROOT.$uriPrev.'">Trước</a></li>';
         
      for($i = 1; $i < $numberPage; $i++) {
         $html .= '<li class="page-item"><a class="page-link" href="'.$this->getLinkPage($page, $i, $queryStr).'">'.$i.'</a></li>';
      }
   ...
   private function getLinkPage($page, $pageChange, $queryStr) {
      $pathInfo = trim($_SERVER['PATH_INFO'], '/');
      $uri = '/'.$pathInfo;
      if(!empty($page)) {
